...
def ShiftRows(state, inv=False):
    if not inv:
        ...
        for i in range(1,len(state)+1):
             state[-i] = state[-i][i+1:] + state[-i][:i+1]
...
def MixColumns(state, inv=False):
    for i in range(4):
        ...
        if not inv:
            ...
            for j in range(4):
                if j==3:
                    state[j][i] = state[j][i] ^ xtime[state[j][i] ^ u] ^ t
                    break
                state[j][i] = state[j][i] ^ xtime[state[j][i] ^ state[(j+1)%4][i]] ^ t
        else:
...
def KeyExpansion(key, Nk, Nb, Nr):
    ...
    for j in range(Nk, Nb * (Nr + 1)):
        temp = [w[i][j-1] for i in range(4)]
        if j % Nk == 0:
            a=temp[0]
            for i in range(0, 4):
                if i==3:
                    temp[i] = S_box[a] ^ rcon[j // Nk]
                    break
                temp[i] = S_box[temp[(i + 1) % 4]] ^ rcon[j // Nk]
        elif Nk>6 and j%Nk==4:
            for i in range(0, 4):
                temp[i] = S_box[temp[i]]
        for i in range(4):
            w[i][j] = w[i][j - Nk] ^ temp[i]
...
def checkAES(key,pt,ct=""):
    ...
    s=''
    for line in aes_enc:
        s+=''.join("\\"+hex(x)[1:] for x in line)
    s=bytes(s,'utf-8')
    print(s)
    print()
    
    if ct=="": ct=s
    #ct=b'\x29\x57\x40\x1A\xC3\x14\x22\x12\x50\x20\x99\xd7\x5f\xf6\xb3\x3a'
...
def pt_changes(aes):
    ...
    key = [random.getrandbits(8) for i in range(Nr)]
